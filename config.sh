...
# and the snapshots/ tooling (docs/SNAPSHOTS.md): this installation's
...
# root (scripts/*.sh or snapshots/*.sh):
#
#   . "$(dirname "$0")/../config.sh"
#
...
# apart — and so two independent instances of this project (or of its
# snapshot tooling) on the same host or the same shared volume get
# non-overlapping paths automatically, from distinct CONTAINER values, with
# no coordination required. Change it only if you are running more than one
...
# yourself. Used by scripts/00-ssh-create-user.sh to set correct host
# ownership on new repo directories via `podman unshare`, and by
# snapshots/77-restore-last-snapshot.sh to re-apply that same ownership on
# restore — which is why these live here rather than in scripts/config.sh
# alone.
BORG_UID=1111
BORG_GID=1111
...
# Snapshot root for the point-in-time snapshot tooling (docs/SNAPSHOTS.md).
...
# dropped. Its shape: a sibling of HOST_REPO_BASE, built from the same two
# values (HOST_STORAGE_BASE, CONTAINER) rather than parsed out of
# HOST_REPO_BASE at runtime -- an operator whose HOST_REPO_BASE does not
# follow that convention must set both independently, or a wrong layout
# would otherwise resolve this onto the wrong volume, which is fatal for a
# mechanism that requires reflink support on the same volume. A trailing
# slash is optional, as above.
