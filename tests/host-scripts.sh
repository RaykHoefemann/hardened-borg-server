...
# The ENFORCED column is 09-show-all-users.sh's verdict, not a second spelling
# of the limit: with clients.conf and the filesystem disagreeing, the current
# block says so in the same words the listing does, and the block that would
# repair it reads "ok".
setup_02
df_stub "$T/repo:$((100 * GIB)):0" \
...
assert "12.6 a change that overcommits the volume is marked as such" $?
...
assert "12.13 an unreadable previous limit is reported, never guessed" $?
