...
> ⚠️ `HOST_CONFIG_BASE` defaults to `${REPO_ROOT}/config` — **inside the git checkout**, and the repository ships no `.gitignore`. A `git add -A` run in the installation directory would commit `clients.conf` and every client public key. The keys are not secret; the client roster and its quotas may well be.
...
- `IMAGE` — the container image reference to run, e.g. `ghcr.io/raykhoefemann/hardened-borg-server:latest`. During the beta phase, `:latest` does not exist yet (no stable release has shipped) — set this to the exact pre-release tag you want to run instead, e.g. `ghcr.io/raykhoefemann/hardened-borg-server:0.1.0-beta.15` (see the [package's version list](https://github.com/RaykHoefemann/hardened-borg-server/pkgs/container/hardened-borg-server/versions) for current tags).
