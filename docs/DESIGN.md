...
- **Whether the configured quotas are the enforced ones** — the same script's `ENFORCED` column compares each client's `clients.conf` entry against the limit the kernel actually applies to its directory, and flags any disagreement. `clients.conf` records an intention; only the filesystem enforces anything, and monitoring the intention alone would report protection that may not exist.
