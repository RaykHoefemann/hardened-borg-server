...
- **Whether the configured quotas are the enforced ones** — the same script reports the limit the kernel actually applies as each client's quota, with the `clients.conf` entry beside it in the `CONFIGURED` column, flagged when the two disagree. `clients.conf` records an intention; only the filesystem enforces anything, and monitoring the intention alone would report protection that may not exist.
...
- **Last Repo Structure Check:** the exact timestamp of the most recent server-side `borg check` of this client's own repository, verbatim from `check-repos.history`, with that run's result token — `(ok)`, `(partial)`, or `(fail)` — in parentheses. It is *structure only*: `--repository-only` never reads the client's archives, so this is not a substitute for the client's own `borg check --verify-data` (3.3, [Client Usage](CLIENTUSE.md) Chapter 8). Omitted entirely until a scheduled check has recorded this repository. A `(fail)` shows the timestamp and token only — never borg's error output; a structural fault is the operator's to triage first (1.2.6). The line is read live from a **per-client** status file (`check-state/<client>`, written by `scripts/20-check-repos.sh`), so the client's session still never touches a file describing any other client (2.2).
