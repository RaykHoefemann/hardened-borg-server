...
**Negative test** — staged 2026-08-30 (against the earlier line wording): with `check-state/<client>` set to a fail state and the repository deliberately corrupted, `ssh … info` returned the fail line and borg's error (`Segment entry checksum mismatch`) did **not** appear; the line was read from the per-client file only. The line's text has since changed to `Last Repo Structure Check: <timestamp> (<token>)`; `tests/wrapper-gating.sh` C6–C9b now exercise the three result states, the absent-line case, the exact-timestamp pass-through, and the "no error text on fail" property with a fake `check-state` file (green). A bench re-run of the corrupted-repo path against the new wording is **not yet staged**.
