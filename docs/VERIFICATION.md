...
**Claim** — [Design](DESIGN.md) Chapter 3.3, [Operations](OPERATIONS.md) Chapter 9.13: `scripts/20-check-repos.sh` runs `borg check --repository-only` against each hosted repository — for one client or all, by hand or on the daily budgeted sweep — so that on-disk corruption (bit rot, a truncated segment, an inconsistent index) is found here rather than by a client at restore time. It needs no encryption key and the server holds none ([Design](DESIGN.md) Chapter 2.1); it never passes `--repair`, on any code path, so a run cannot modify a repository; and one repository failing does not stop the sweep — the exit status is `1` if any repository did not come back clean.
...
Five checks, staged on the FCOS bench 2026-08-30 against two disposable clients — `bc1` populated from a **borg 1.2.8** client (Linux Mint), `bc2` from a **borg 1.4.0** client (Debian), the server image's own borg being 1.4.0. 13A: the check completes with no key on the server. 13B: a run leaves a healthy repository's stored data byte-for-byte unchanged. 13C: a deliberately corrupted segment is reported *and* the sweep still finishes. 13D: the borg process runs as the unprivileged container user. 13E: the `info` channel's `Last Repo Structure Check:` line shows the exact `check-repos.history` timestamp with its `(ok)`/`(partial)`/`(fail)` token, isolated per-client, and never carries borg's error text.

All five were re-run 2026-08-30 on branch `feature/borg-check` @ `f1c1c9d` — clients `mc` (borg 1.2.8, Mint) and `dc` (borg 1.4.0, Debian) against instance `BorgTestC` on image `hbs-nachverif13:wrap` (released `@sha256:bdd01e62…` + this branch's `borg-wrapper.sh`). This is the run that stages 13E's negative test against the current line wording (`Last Repo Structure Check: <ts> (<token>)`) and confirms the human-readable `check-repos.history` size field and the full-timestamp `check-state`. Per-check evidence is in each entry's blockquote; the working notes are `testlab/Pruefbericht-nachverif13-2026-08-30.md`.
...
>
> Re-verified 2026-08-30 on branch `feature/borg-check` @ `f1c1c9d` (instance `BorgTestC`, image `hbs-nachverif13:wrap` = released `@sha256:bdd01e62…` + this branch's `borg-wrapper.sh`; clients `mc` from borg 1.2.8, `dc` from borg 1.4.0): `./scripts/20-check-repos.sh mc` and `… dc` each `FULL pass, no problems found.`, exit `0`, `env | grep BORG` empty, no prompt, no hang.
...
>
> Re-verified 2026-08-30 @ `f1c1c9d` (`mc`, borg 1.2.8): 10 segment files under `mc/data/`, `sha256sum` list identical before and after `./scripts/20-check-repos.sh mc`; `lock.exclusive` gone by the time the script returned. Negative direction: `podman exec --user borg … borg check --repository-only --repair /repo/mc` (9.14 procedure, `BORG_CHECK_I_KNOW_WHAT_I_AM_DOING=YES`) reported `attempting to recover data/0/2` / `errors found and repaired.` and the hash of `data/0/2` changed — the read-only sweep never touches a segment, `--repair` rewrites one.
...
> Re-staged 2026-08-30 @ `f1c1c9d`: 128 bytes overwritten at offset 4096 of `mc`'s largest segment (`data/0/2`, 55 MB); `CHECK_CYCLE_DIVISOR=1 ./scripts/20-check-repos.sh` reported `mc: failed` with `Data integrity error: Segment entry checksum mismatch [segment 2, offset 8]` indented and the `Do NOT run 'borg check --repair' blindly … OPERATIONS.md 9.14` guidance, continued to `dc: FULL pass`, printed `this run: 1/2 checked clean` / `1 repository/-ies did NOT come back clean`, exited `1`. The history line for `mc` carried the `fail` token; `check-state/mc` became `fail  <now>  <earlier full-pass ts>` — the last full-pass timestamp (field 3) was **kept**, not overwritten by the failure.
...
> Re-verified 2026-08-30 @ `f1c1c9d`: `grep -n 'podman exec' scripts/20-check-repos.sh` shows `--user borg` on both borg-check invocations; `podman exec --user borg BorgTestC id` → `uid=1111(borg)`; `mc/config` on the host owned `525398:525398`. Without `--user`: `id` → `uid=0(root)`, and a file it created under `/repo/mc` came out `1000:1000` on the host.
...
**Claim** — [Design](DESIGN.md) Chapter 2.4: the client `info` output carries one `Last Repo Structure Check:` line about the client's *own* repository — read live from a per-client `check-state/<client>` file, never the multi-client `check-repos.history` — that shows the exact timestamp of the last check with its result token `(ok)` / `(partial)` / `(fail)`, and never puts borg's error text in front of a client.

**Why it matters** — the line is a checkability signal (like the version), but a `--repository-only` "pass" is not an archive-content guarantee; a client that reads it as one skips its own `borg check --verify-data`. And a line read from a file describing every client is a cross-visibility regression ([Design](DESIGN.md) Chapter 2.2), which the per-client file is there to prevent. The timestamp must match `check-repos.history` exactly — the file it comes from.
...
**Pass** — an `ok` state produces `Last Repo Structure Check: <timestamp> (ok)`, the `<timestamp>` byte-for-byte the last field-2 value in `check-repos.history` for this client. A `partial` produces `… (partial)`, a `fail` produces `… (fail)` — timestamp and token only, **no** borg output. With no scheduled check yet, the line is **absent** — not a placeholder claim. On the host, `HOST_LOG_BASE/check-state/` holds one file per current client and nothing else.

**Fail** — the line carries anything beyond `<timestamp> (<token>)`; the timestamp disagrees with `check-repos.history`; borg's `Data integrity error …` reaches the client; or the line appears when no check has run.

> Verified 2026-08-30 @ `f1c1c9d`: before any sweep, `ssh … info` from both `mc` and `dc` carried **no** `Last Repo Structure Check:` line. After `CHECK_CYCLE_DIVISOR=1 ./scripts/20-check-repos.sh`, each client's `info` showed `Last Repo Structure Check: 2026-08-30T15:31:54Z (ok)` — byte-for-byte the field-2 value of that client's own last `check-repos.history` line. Re-checking **only** `dc` moved `dc`'s line to `…T15:32:19Z (ok)` while `mc`'s stayed at `…T15:31:54Z` — each `info` reflects its own `check-state/<client>` file, never the shared history.

**Negative test** — staged 2026-08-30 @ `f1c1c9d` against the corrupted repository from 13C: `ssh … info` from `mc` returned exactly `Last Repo Structure Check: 2026-08-30T15:32:34Z (fail)` — timestamp and token only. Grepping that output for `integrity` / `checksum mismatch` / `segment` / `errors found` / `rebuilt index` found nothing: borg's error stayed server-side. `dc`'s line was unaffected (`… (ok)`). `tests/wrapper-gating.sh` C6–C10c additionally exercise the three result states, the absent-line case, the exact-timestamp pass-through, and the malformed/empty/unknown-token `check-state` file in CI.

**What this does not show** — that the check itself discriminates (13A–13D) or that the timestamp is accurate (it is whatever `20-check-repos.sh` last wrote). This is only that the line is correctly scoped, isolated, and conservative.
