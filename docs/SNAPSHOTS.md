...
| `77-restore-last-snapshot.sh` | `rm -rf`, `cp -a --reflink=always`, `du -sh`, plus (via `scripts/lib.sh`) `xfs_quota` and `podman unshare` | Same reasoning as above for the live directory, plus the same project-id/ownership mechanics `00-ssh-create-user.sh` uses. `du -sh` needs the same `sudo` as `75-`'s (issue #35) — it sizes the same generation directory before showing it in the pre-delete confirmation. |
