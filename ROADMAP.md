...
Automatic, operator-side, point-in-time snapshots of the **entire storage volume** that carries the repositories, with a retention policy and a restore path.
...
Without snapshots, every one of these ends in the same place: a full restore from the offsite mirror (11.2). That is slow, depends on a third party's availability, and is a disproportionate response to what is usually a small, local, entirely recoverable mistake. Snapshots turn it into a local rollback measured in seconds instead.

**This is a small, local piece of the picture, not a substitute for it.** Snapshots live on the same storage as the data they protect — not a second copy, and no defense against media failure, site loss, or an attacker holding root on this host (root can clear the immutable flag as easily as anything else here). Offsite mirroring (11.2) with an append-only-enforcing target remains mandatory regardless of whether this item ships.
...
> - **A separate project entirely**, vendored or referenced from here. Rejected: coupling it to this project's release cycle, `VERSION`, CI and Docker image would be an odd provenance for anyone who'd want to reuse it for an unrelated container if it were genuinely factored out — but at the size 11.5 actually turned out to need (four scripts: creation, prune, listing, restore — see "Mechanism" below), the real cost was duplicating infrastructure this repo already has (`config.sh`/`lib.sh` conventions, the `tests/host-scripts.sh` stub pattern, the release/CI machinery) for no proportionate benefit.
>
...
### Copy ordering — verified: order does not matter

A crash-consistent read of a Borg repository — the state any hard host crash mid-write leaves behind — is exactly what Borg's append-only, commit-tagged segment design is built to tolerate: an interrupted transaction is simply invisible until it is retried, and `borg check` recovers the rest. A `cp -a --reflink=always` walk over a live, growing repository tree is not the same guarantee on its face: it is not atomic across files, and can span real wall-clock time on a large repository.

The concern this raised: if the walk copies `data/` (segments) before `index`/`hints`, and a transaction commits in between, the resulting snapshot could contain an index that names a segment the snapshot never actually captured — a dangling reference, which is not a state Borg's crash recovery is designed to see (it assumes segments can outrun the index, never the reverse). Copying `index`/`hints`/lock files before `data/` was the proposed mitigation, on the reasoning that any segment the snapshot then picks up ahead of what the copied index already knew about is just the ordinary "segment exists, transaction not yet indexed" case Borg already handles.

**Tested empirically (2026-08-25, Borg 1.2.8) and the concern did not materialize, in either direction:**

- A deterministic worst case was built directly — an `--encryption=none` throwaway repo with archive A committed, then `data/` copied (capturing only A's segments), then a second archive B committed, then `index`/`hints`/`integrity`/`config` copied (now referencing both A and B) — i.e. exactly the "index copied after data" ordering the concern was about, taken to its logical extreme (a whole archive apart, not just a race window). Against a **fresh** client (`BORG_CACHE_DIR`/`BORG_SECURITY_DIR` pointed at an empty directory, to rule out a client's cache from a previous, newer access to the same repo ID masking or mimicking the result — see the pitfall noted below), `borg check`, `borg check --verify-data` and `borg list` all completed with zero errors. `list` correctly reported only archive A: the honest, complete state that `data/` actually contained, not a dangling reference to B.
- The reverse (deliberately "correct") ordering — `index`/`hints` copied while only A existed, `data/` copied after B committed — also completed cleanly, correctly listing both A and B.
- The realistic case was then reproduced directly rather than only reasoned about: a real `borg create` of 300 MB incompressible data was run in the background, and a **plain, unordered** `cp -a` (no `--reflink`, no special-casing of any file) was taken against the whole repository tree while it was actively writing — confirmed genuinely mid-write by comparing segment file sizes afterward (one segment copied at 12.6 MB against a final 314 MB, plus a copied `lock.exclusive`). `borg check -v` recognized and cleared the stale lock, silently and correctly excluded the interrupted archive, and reported both prior archives intact with no problems.

The likely mechanism: on a cache-less first access, Borg does not appear to trust the shipped `index.N`/`hints.N` at all — it rebuilds the true state directly from the segments present in `data/` (replay), which is also what makes an interrupted transaction invisible after a real crash. If that holds, no copy order can produce a dangling reference, because the shipped index is never taken as ground truth in the first place. This is inferred from observed behavior across these three runs, not confirmed against Borg's source.

**Caveats, stated plainly:** run on ext4 with plain `cp` (this test environment has no XFS reflink volume — but the question under test is copy *timing* relative to a live writer, which reflink does not change: it only changes how blocks are stored once copied, not when the directory walk observes file state); `--encryption=none`, not `repokey`; one trial per scenario, not a stress test across many interleavings. This rules out the specific mechanism of harm the project's own reasoning proposed, and reproduces the realistic case once successfully — it is not a proof that no interleaving anywhere can misbehave.

**Practical conclusions for the tooling:**

- `70-create-snapshot.sh` (below) does a single, unordered `cp -a --reflink=always` over each client's tree. No index/hints-before-data special-casing.
- **Pitfall for any future tooling that runs `borg` commands directly against a snapshot** (neither `75-list-snapshots.sh` nor `77-restore-last-snapshot.sh` do — both work at the filesystem level only), found while ruling out a false positive above: a client whose local Borg cache (`~/.cache/borg/<repo-id>/`) was built against the live repository will refuse most operations against an older snapshot of the same repository with "Cache is newer than repository" — this fires on a perfectly valid older snapshot exactly as it would on a corrupted one, so it is not itself a corruption signal. Restoring or inspecting a snapshot needs its own cache (e.g. a scratch `BORG_CACHE_DIR`), not the operator's or a client's ordinary one.

### `70-create-snapshot.sh` — verified end to end on a real deployment

Beyond the copy-ordering test above (a throwaway, purpose-built repository), the finished script was run against a real deployment: `snapshots/70-create-snapshot.sh` and `snapshots/71-timer-install.sh` against the FCOS server VM described in [Test Environment](../docs/TESTENV.md) Part B, 11 real client repositories (`09-show-all-users.sh`'s own roster, spanning both groups and several `-repokey` clients deliberately left in the unusable state [Verification](docs/VERIFICATION.md) test 8 produces), on the VM's real XFS volume with `reflink=1`, `sudo` genuinely elevating to root (unlike every earlier test run of this script, which used a mocked `sudo` — this dev environment has neither passwordless sudo nor an XFS reflink volume to test against directly).

What this confirmed, measured rather than assumed:

- **Ownership is preserved correctly.** Every client directory's uid:gid (the container's mapped subuid) matched exactly between the live repository and its snapshot — confirming `sudo cp -a` was the right call over a plain `cp -a` (this script's own header already reasoned through why; this is that reasoning checked against a real mapped-subuid volume rather than assumed).
- **The immutable flag is a real barrier, not merely a flag that reads back correctly.** `sudo rm -rf` against a completed snapshot — root, not the rootless container — failed with `Operation not permitted` on every single segment file. `lsattr -d` before that confirmed the flag was set; this is the actual deletion attempt the flag exists to stop, not just a readback.
- **Reflink cost is negligible at real scale.** 11 client repositories snapshotted twice (22 generations total) moved the volume's reported usage from what the 11 live repositories alone accounted for to a total of 2.0 GiB — consistent with the throwaway-repository measurement above, now against real client data rather than a synthetic 250 MB tree.
- **Per-client timing is fast enough that the whole run barely registers.** 11 clients: 0.73 s end to end (~0.06 s/client) invoked directly, 1.12 s (~0.09 s/client) invoked through `systemd`. `chattr -R` — the step expected to dominate on a large tree — did not show up as a bottleneck at this client count; unmeasured is whether that holds for a single client repository large enough to matter (a multi-terabyte archive with many segments), which is the case the header's own "TIMING" note flags as unmeasured.
- **`journalctl --user -u snapshot-create.service` correctly captures the script's own stdout** on this VM — including every individual `sudo cp`/`sudo chattr` invocation, since `sudo` itself logs each call. (A different, apparently host-specific journald quirk suppressed a unit's own stdout during an earlier, aborted attempt to test this on an unrelated desktop machine — noted here only because it briefly looked like a bug in the unit files and was not one.)
- **A prune script will need `sudo rm`, not only `sudo chattr -R -i`.** Clearing the immutable flag was not sufficient to delete a snapshot as the operator: the reflinked files keep the *original* client directories' restrictive mode and mapped-subuid ownership, so an unprivileged `rm -rf` failed with `Permission denied` (a distinct failure from the immutable flag's `Operation not permitted`) until root actually removed them. Worth stating plainly for whoever writes the prune script next: the whole deletion, not only the flag change, needs the elevation — see the note added to "Constraints to preserve" below.

The VM was returned to its pre-test state afterward (both snapshot generations removed, the timer uninstalled, `config.sh` restored) rather than left as a standing fixture — see [Test Environment](../docs/TESTENV.md) Part B for how to reach the same lab again.
...
### Snapshot comparison — considered and dropped

An earlier draft of this item proposed comparing two snapshot generations structurally (segment path/size/mtime, `config`/`nonce`) as a key-less integrity tripwire, on the reasoning that under enforced append-only an existing segment changing is never legitimate:

- new segments, latest one grown — ordinary backup traffic
- an **existing** segment changed in size or mtime — cannot happen under append-only; would indicate tampering or a bug
- segments missing — only `borg compact` removes segments, so outside a deliberate operator run this would be an anomaly
- `config` or `nonce` changed — always worth investigating

On reconsideration this does not hold up against either threat it was meant to serve:

- **Against a compromised client** — the actual scenario `75-list-snapshots.sh` / `76-delete-snapshots.sh` / `77-restore-last-snapshot.sh` exist for — the signal above cannot occur in the first place. Append-only (Chapter 1.2.4) already prevents a client from modifying or removing an existing segment over the Borg protocol; the only thing a compromised client can do is append illegitimate new segments/generations, which already shows up as abnormal growth in `75-list-snapshots.sh`'s size column. A structural diff between two snapshots adds nothing on top of that.
- **Against the server's own mutating operations** (`borg check --repair`, 11.3; reclamation tooling, 11.1) — every legitimate run of either trips the exact same signal a real problem would. Unable to tell "the repair did what it should" from "the repair broke something", the check would be noise, not a diagnosis.

What survives that scrutiny is a narrow, largely theoretical case: host-level write access below root (root is already out of scope, see "Scope" above) swapping an existing segment for a replacement that is itself internally well-formed — invisible to `borg check --repository-only` as well, for the same reason. Not judged worth building and maintaining custom tooling for on its own.

Snapshots without this check remain exactly what "Scope" above already says they are: a fast, local way for the operator to undo their own mistake, with limited, honest protection against unprivileged host-side damage — not a tamper-detection system. That job stays with offsite mirroring (11.2), mandatory regardless of whether this item ships.
...
- **The prune path is the most dangerous code in the deployment.** Removing an expired snapshot requires clearing the immutable flag first, so a script whose job is to disarm protection and then delete recursively necessarily exists. It must validate that its target resolves inside the snapshot root and refuse everything else, rather than trusting its argument. Confirmed against a real deployment (see "`70-create-snapshot.sh` — verified end to end" above): clearing the flag is not sufficient on its own to let the operator delete a snapshot — the reflinked files keep the *original* client directories' mapped-subuid ownership and mode, so the deletion itself needs `sudo`/root too, the same as the flag change. A prune script that clears the flag with `sudo` and then runs a plain `rm -rf` will fail partway through with `Permission denied`, having already disarmed the protection on whatever it did reach.
...
- **Operator-side only (Chapter 1.2.6).** Creation, listing, deletion, and restore are host-side actions. No client-facing interface, no new port, and nothing surfaced through the `info` channel — snapshot existence and timing are operational metadata that clients have no business seeing.
...
Neither script covered "directory and project id present and correct, `clients.conf` entry missing." **`04-reattach-client.sh` (Chapter 9.2.1) closes that gap:** it reads the group, XFS project id and enforced quota back off the directory — the same values `09-show-all-users.sh` and `02-change-user-quota.sh` already read — and writes the `clients.conf` line and, unless one already survived, an empty key file. It needs neither `podman unshare` nor a mutating `xfs_quota` call, since nothing on the filesystem is created or changed; only the read-only enforcement check 00/02/03 all make runs via `sudo`. It refuses rather than guesses wherever the state on disk is not exactly "complete except for the declaration" — see Chapter 9.2.1 for the specific cases, including an enforced quota that is not a whole number of GiB, which `clients.conf`'s `<n>G` format cannot represent and which this script will not round.
