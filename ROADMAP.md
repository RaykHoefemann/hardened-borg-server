...
## 11.9. Debian Security Status Monitoring

`tests/package-pin-freshness.sh` (1.1.2) answers "has Debian's package pool moved past the pinned version" by comparing the pin against `apt-cache policy`'s Candidate. It does not answer "is the version currently pinned now known to be vulnerable" — those are different questions, and a security advisory can be open against an *unchanged* version string for a while before a fix is packaged.

Not a defect in the freshness check; it is honest about what it measures. Debian's own fix mechanism — a new upload once an advisory is triaged — is still the right mechanism, and the freshness check catches that upload the moment it lands, the same way it catches any other package movement. The gap is only the window between an advisory opening and a fix being packaged: during that window there is nothing new to compare against, so the check correctly reports "ok" while a known issue sits open. This is not hypothetical — CVE-2026-62268 against `borgbackup` sat as "needs triaging" on the Debian security tracker while `apt-cache policy`'s candidate stayed exactly at the pinned `1.4.0-5` (see [Design](docs/DESIGN.md)/[Verification](docs/VERIFICATION.md) history around the 1.1.2 release for the fuller scrutiny of that CVE against this project's own threat model, which found it inapplicable here regardless: the vulnerable code path is `borg extract`, never run server-side, and needs the client-held key this server never holds).

A future check could query `security-tracker.debian.org`'s machine-readable feed for open CVEs against the pinned packages, independent of version comparison, so an open advisory is visible before Debian repackages. Not planned — noted here so the freshness check's actual scope stays honest rather than being assumed to cover this too.

## 11.10. Explicit Threat-Model Treatment of `/config` as a Runtime Trust Boundary

Raised during an external repo assessment (L-01): `HOST_CONFIG_BASE` (bind-mounted as `/config`) holds `clients.conf`, `config/keys/`, `server_info.conf`, and — the one that matters most — `config/ssh_host_keys/`, the server's private SSH host key ([Operations](docs/OPERATIONS.md) Chapter 9.1's `⚠️` note). It is writable from inside the running container for the life of the process. [Design](docs/DESIGN.md)'s threat model does not yet ask, in one place, exactly what a fully compromised `borg serve` session could do to it.

The container itself runs as root (the image sets no `USER`), but a client session is not the container process — `sshd`'s `AllowUsers borg` plus pubkey auth de-escalates every client connection to the unprivileged `borg` user (uid/gid baked in at build time, Dockerfile) before the forced command (`borg-wrapper.sh`, then `borg serve --restrict-to-path --append-only`) ever runs. So the question is what uid `borg` can do to `/config`, not what root could do.

One concrete data point worth starting from: `build_authorized_keys.sh` writes both `authorized_keys` and each client's rendered info text as `chown borg:borg` / `chmod 600` (lines around 259-260 and 330-331). The `borg` user therefore *owns* part of what governs its own future connections. Whether a compromised `borg serve` process could reach and rewrite its own `authorized_keys` — and, if so, what that would actually buy an attacker beyond what `--restrict-to-path`/`--append-only` already confine — is the specific question nobody has walked through yet.

Explicitly **not** a call to change anything reflexively. Current status, in the assessment's own words, is a deliberately accepted residual risk. The ask is to walk the question through in [Design](docs/DESIGN.md)'s threat model — what can a fully compromised `borg` process reach under `/config`, and does any of it change the guarantees Chapter 2 makes to other clients — and record the answer, whether that answer is "already fine, here is why" or "needs a mitigation."

## 11.11. Per-Client OS Accounts Inside the Container

Related to 11.10 but a different question. Every client currently authenticates as the same single `borg` Unix user (uid/gid baked in at build time); what confines one client's session to its own repository is entirely software — the forced command, SSH `restrict`, and Borg's own `--restrict-to-path` ([Design](docs/DESIGN.md) 1.2.1, 2.2). There is no OS-level (Unix permission / uid) boundary *between clients*, only between the container and the host.

[Design](docs/DESIGN.md) 1.1.2 already lists "Cross-client isolation failure" as a threat scenario and answers it with "Host-level separation provides an additional enforcement boundary" — but that boundary (SELinux, rootless Podman) separates the *container* from the *host*, not one client's session from another's *inside* the same container. If a bug in `--restrict-to-path` itself let a compromised `borg serve` session open a path outside its assigned repository, nothing at the Unix-permission level would necessarily stop it, since every client's process runs as the identical uid.

To investigate: giving each client its own Unix account inside the container (mapped per-client at provisioning time, `00-ssh-create-user.sh` territory) so that even a `--restrict-to-path` failure would still hit a permission-denied at the filesystem level for every repository except the session's own. Not evaluated yet — cost, how `sshd`'s single `AllowUsers borg` and the shared `authorized_keys` file would need to change, whether per-client accounts still compose with the existing XFS project-quota-per-repo model, and whether this duplicates protection `--restrict-to-path` already provides in the common case versus adding a second, independent layer for the case where it doesn't. Recorded as a question to investigate, not a design or a commitment.
