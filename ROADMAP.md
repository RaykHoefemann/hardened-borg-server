...
- **2** — deliberately disabling or bypassing `borg-wrapper.sh` on a live deployment and confirming a forbidden command actually executes. Attempted once already; blocked at the tooling level of the session that attempted it, not by a deployment or a decision — still open.
- **3A** — a recipe exists ([Test Environment](docs/TESTENV.md) "Break this" table: a key without `command=`, and separately one missing only `,restrict`) and is asserted by `tests/authorized-keys-generation.sh` against the generator's code, but not against a live `authorized_keys` file.
- **4A / 4C** — this project has no supported path to a rootful deployment at all, not merely a second configuration of the existing one: `systemctl --user`, `sudo podman` vs. plain `podman`, and the `podman unshare` uid-mapping this repo relies on elsewhere (test 6, the 5.5A recipes) are baked into `scripts/50-service-install.sh` through `99-container-status.sh`. Staging this check's `Fail` state would mean standing up a bare rootful container by hand, entirely outside this repo's own tooling — which would confirm the check's *commands* (`podman info`, the `ps` line) read correctly under rootful podman in general, but would not be a statement about this project running rootful, since it cannot.
- **5.5A / `unreadable`** — both host-side `chmod` candidates named in [Test Environment](docs/TESTENV.md) chapter 8 have now been staged, and neither reaches `unreadable`. The group-directory recipe (#33) fails because that directory is left owned, via `podman unshare`'s uid-0 mapping, by the same host user that runs the reporting script, so tightening it never reaches the script's own access — it silently locks clients out under a report that stays green instead. `chmod` on `HOST_REPO_BASE` itself — that chapter's "Hypothesis, not yet staged" candidate — was staged next and produces `MISSING on host` for the same underlying reason: `report_for()` in `09-show-all-users.sh` checks `[ -d "$d" ]` before it runs `df -kP "$d"`, and both calls need the same search permission on the same parent directories, so any `chmod` that breaks one breaks the other first — the `unreadable` branch sits structurally behind a check that a permission change always trips before it. The honest conclusion stands: this branch is reachable only by a genuine `statfs()` failure (an unmounted volume, a stale handle), not by any host-side permission change. That still belongs in `VERIFICATION.md` 5.5A rather than another invented recipe; whether to file it as an issue is a project decision, not made here.
