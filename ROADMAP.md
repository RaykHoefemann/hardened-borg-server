...
- **Pitfall for the future restore/list/compare tooling, found while ruling out a false positive above:** a client whose local Borg cache (`~/.cache/borg/<repo-id>/`) was built against the live repository will refuse most operations against an older snapshot of the same repository with "Cache is newer than repository" — this fires on a perfectly valid older snapshot exactly as it would on a corrupted one, so it is not itself a corruption signal. Restoring or inspecting a snapshot needs its own cache (e.g. a scratch `BORG_CACHE_DIR`), not the operator's or a client's ordinary one.
