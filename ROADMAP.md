...
- A snapshot is a copy-on-write copy of every top-level directory of `HOST_REPO_BASE` into `.snapshots/<timestamp>/`, made with `cp -a --reflink=always`. Blocks are shared until they diverge, so a snapshot costs almost nothing at creation.
