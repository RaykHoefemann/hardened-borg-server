...
- **4A / 4C** — a rootful Podman install and a deliberately rootful container, each confirmed to produce this check's documented `Fail` output. Deliberately not attempted alongside the others above, since it means standing up a second, differently configured install rather than staging a state on the existing one.
- **5.5A / `unreadable`** — the one item here that is not simply unstaged: the first recipe tried for it (`chmod` on a group directory) was staged, and turned out wrong (#33). The group directory is left owned, via `podman unshare`'s uid-0 mapping, by the same host user that runs the reporting script, so tightening it never reaches the script's own access — it silently locks clients out under a report that stays green instead. A second candidate, `chmod` on `HOST_REPO_BASE` itself, is named in [Test Environment](docs/TESTENV.md) chapter 8 as untested. If that one also fails to reach `unreadable`, the honest conclusion is that this branch is reachable only by a genuine `statfs()` failure, not by any host-side permission change — and that belongs in `VERIFICATION.md` 5.5A, not another invented recipe.
- **5.5A / empty `HOST_REPO_BASE`** — unlike the row above, this one is a plain configuration state, not a permission puzzle: blank the `HOST_REPO_BASE` variable in `config.sh` and run `09-show-all-users.sh`. `lib.sh` already has the branch for it (`n/a (!) (HOST_REPO_BASE not set or not accessible)` on the `Disk usage:`/`Disk free:` lines) — this just needs staging.
- **8, reverse direction** — the **Run** in test 8 already stages the broken-mode repository and confirms the *correct* wrapper refuses it. What remains is disabling the encryption gate itself in a live deployment and confirming this check would then let `borg list` succeed. `tests/wrapper-gating.sh` covers that against the code, not against a running deployment.
